...
ux = unique(x);
dots_x = randsample(ux,numDots,false);

uy = unique(x);
dots_y = randsample(uy,numDots,false);

%dotPositionMatrix = [reshape(dots_x, 1, numDots); reshape(dots_y, 1, numDots)]
dotPositionMatrix_x = [reshape(dots_x, 1, numDots)];
dotPositionMatrix_y = [reshape(dots_y, 1, numDots)];
%original
...
dotColor1 = [0 0 1]; %blue
dotColor2 = [1 1 0]; %yellow
...
%for i = 1: (tot-yellow_dots)
for i = 1: tot
    if i <= blue_dots
%Screen('DrawDots', window, [dotXpos dotYpos], dotSizePix, dotColor, [], 2);
    Screen('DrawDots', window, [dotPositionMatrix_x(i) dotPositionMatrix_y(i)], dotSizePix, dotColor1, dotCenter, 2);
    else 
        Screen('DrawDots', window, [dotPositionMatrix_x(i) dotPositionMatrix_y(i)], dotSizePix, dotColor2, dotCenter, 2);
    end
...
%imageArray = Screen('GetImage', window);
  ...
  % GetImage call. Altered the rect argument to change the location of the screen shot
    imageArray = Screen('GetImage', window, [xCenter-100 yCenter-200 xCenter+1300 yCenter+1200]);
    
    %%Save image to file. 
    k=1;
    filename = sprintf('%d_blue_%d_yellow.jpg',blue_dots,yellow_dots);
